...
<!-- Bootstrap 3.3.7 -->
 ...
 <!-- DEPENDENT SELECT SCRIPT FOR STUDENT REGISTRATION IN THE ADMIN PORTAL -->
  <script type="text/javascript">
    $('#school').on('change',function(){
            ...
            if(sch_id){
                $.ajax({
                    type:'POST',
                    url:'<?php echo base_url('Sections/ajax_request');?>',
                    data:'sch_id='+sch_id,
  ...
  <script type="text/javascript">
	...
	{
		$.ajax({
			url:"<?php echo base_url(); ?>Sections/load_data",
			method:"POST",
	...
	$('#import_csv').on('submit', function(event){
		...
		$.ajax({
			url:"<?php echo base_url(); ?>Sections/import",
			method:"POST",
		...
		swal({
			...
			preConfirm: function() {
			  return new Promise(function(resolve) {
			     ...
			     $.ajax({
			   		url: '<?php echo base_url("Sections/DeleteStudArares"); ?>',
			    	type: 'POST',
			       	data: "id":id,
			       	dataType: 'json'
			     })
...
function validate(a)
{
    ...
    swal({
            ...
            closeOnConfirm: false }, function()
        {
            ...
            $(location).attr('href','<?php echo base_url()?>Sections/DeleteStudentArares/'+id);
        }
    );
}*/
 </script>
