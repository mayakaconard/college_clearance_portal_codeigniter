...
class Admin_model extends CI_Model {
    ...
    public function getStudentArares() {
        $this->db->select('*');
    ...
    public function UnclearedStudents() {
        $sql = "SELECT * FROM section_arares WHERE status_id=2";
        $query = $this->db->query($sql);
        return $query->num_rows();
    }

    //count cleared students
    public function ClearedStudents() {
    ...
    /* Add section Administrators */
    public function add_admin($admin) {
        $qry = $this->db->insert('users', $admin);
        if ($qry) {
            return true;
        } else {
            return false;
